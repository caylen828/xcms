<?php
$tmpcid="";
$datas=getContentsByChid(82,$db,100,0,"cid,c_title");
if ($datas){
...
<?php
if ($tmpcid!=="" && $data=getContentById($tmpcid,$db))
{

  $atts=getAttachmentByCid($tmpcid,$db);  //获取所有附件，并以title为key放入一个数组
  $attArray=array();
    if ($atts){
...
<?php
  ...
  echo '<center><h1>首页数据为空！</h1></center>';
}
?>
  </div>
