	...
	*/
	function insert_data($table,$array)
	{
		...
		foreach($array as $v)
		{
			...
			{
				$value=$value.",'".str_replace($vReplace1,$vReplace2,$v)."'";
			}
		}
...
/*
	...
	目前只支持MYSQL的批量插入
	*/
	function insert_datas($table,$array)
	{
		if (!is_array($array)) return false;
		$sql="";
		$vReplace1=array("\\","'");
		...
		foreach($array as $v)
		{
			...
			foreach($v as $v1)
			{
				...
				{
					$value2=$value2.",'".str_replace($vReplace1,$vReplace2,$v1)."'";
				}
			}
	...
	/**************************
		...
		$table  要修改的表名
		$array  被修改的数据
		$where  修改条件，一维数组就是KEY VALUE匹配。如果是字串就直接使用。
	...
	***************************/
	function update_data($table,$array,$where)
	{
		...
		foreach($array as $k => $v)
		{
			if (is_string($v))
			{
				$set=$set.",".$k."='".str_replace($vReplace1,$vReplace2,$v)."'";
			}else
			{
				$set=$set.",".$k."=".$v;
			}
		...
		if (is_array($where))
		{
			foreach($where as $k => $v)
			{
				if (is_string($v))
				{
					$whereString=$k."='".str_replace($vReplace1,$vReplace2,$v)."'";
				}else
				{
					$whereString=$k."=".$v;
				}
		...
		$sql=$sql." set ".$set." where ".$whereString;
